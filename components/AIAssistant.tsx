...
export const AIAssistant: React.FC<AIAssistantProps> = ({ subjects, currentSubjectId }) => {
  ...
  useEffect(() => {
    ...
    const loadState = () => {
        ...
        const savedChat = localStorage.getItem('omni_chat_history');
        if (savedChat) {
            try {
    ...
    // Add Sync Listener
    window.addEventListener('omni_sync_complete', loadState);

    const interval = setInterval(updateTimeOfDay, 60000); // Update every minute
    return () => {
        clearInterval(interval);
        window.removeEventListener('omni_sync_complete', loadState);
    };
  }, []);
  ...
  useEffect(() => {
      ...
      // Optionally sync here, but debouncing would be better. For now, we rely on the periodic sync or on mount/unmount logic, 
      // but the prompt asked to ensure it syncs. We can trigger it here.

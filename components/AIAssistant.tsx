...
export const AIAssistant: React.FC<AIAssistantProps> = ({ subjects, currentSubjectId }) => {
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    try {
        const saved = localStorage.getItem('ekagrazone_chat_history');
  ...
  useEffect(() => {
    ...
    const loadState = () => {
        const savedCustom = localStorage.getItem('ekagrazone_custom_personas');
        if (savedCustom) {
            try {
        ...
        const savedPersona = localStorage.getItem('ekagrazone_ai_personality');
        if (savedPersona) {
        ...
        const savedChat = localStorage.getItem('ekagrazone_chat_history');
        if (savedChat) {
            try {
            ...
            // Handle factory reset case where key is removed
            setMessages([]);
        }
    };
    ...
    // Add Sync Listener
    window.addEventListener('ekagrazone_sync_complete', loadState);

    const interval = setInterval(updateTimeOfDay, 60000); // Update every minute
    return () => {
        clearInterval(interval);
        window.removeEventListener('ekagrazone_sync_complete', loadState);
    };
  }, []);
  ...
  useEffect(() => {
      localStorage.setItem('ekagrazone_chat_history', JSON.stringify(messages));
  ...
  const selectPersonality = (id: string) => {
      setPersonality(id);
      localStorage.setItem('ekagrazone_ai_personality', id);
      dbService.syncSettingsToCloud().catch(console.error);
  ...
  const handleSaveCustomPersona = () => {
      ...
      localStorage.setItem('ekagrazone_custom_personas', JSON.stringify(updated));
      dbService.syncSettingsToCloud().catch(console.error);

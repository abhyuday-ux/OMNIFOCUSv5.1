    ...
    <div className="space-y-6">
        ...
        <div className="p-4 bg-white/5 backdrop-blur-md rounded-xl border border-white/10 flex items-center justify-between">
             ...
             {isGuest ? (
                 ...
                 <button onClick={logout} className="p-2 bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white rounded-lg transition-colors">
                     <LogOut size={18} />
                 </button>
             )}
        </div>
        ...
        {/* Space Mode & Zen Mode */}
        {/* ... (Kept similar to original code, just wrapped in div for brevity) ... */}
        ...
        <main className="flex-1 relative overflow-hidden flex flex-col">
           ...
           {activeTab === 'timeline' && (
             <div className="flex-1 flex flex-col px-4 overflow-y-auto pb-4">
                ...
                <HistoryList sessions={selectedDateSessions} subjects={subjects} lifetimeTotalMs={lifetimeTotalMs} averageSessionMs={averageSessionMs} />
             </div>
           )}
           ...
           {activeTab === 'calendar' && (
             <div className="flex-1 px-4 overflow-y-auto pt-4">
               ...
               <div className="mt-8">
                 <GoalChecklist dailyTotalMs={dailyTotalMs} goals={dailyGoals} onGoalUpdate={loadGoals} selectedDate={selectedDate} targetHours={targetHours} />
               </div>
             </div>
           )}
           ...
           {activeTab === 'settings' && (
             <div className="flex-1 p-6 overflow-y-auto">
               ...
               <div className="mt-12 text-center pb-8">
                 <p className="text-slate-500 text-[10px] font-mono font-medium tracking-wide">Made by Abu Dhabi (ABHYUDAY)</p>
               </div>
             </div>
           )}
        </main>
                    ...
                    <div className="h-full grid grid-cols-12 gap-8 animate-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="col-span-4 flex flex-col gap-6 overflow-y-auto pr-2">
                             <div className="bg-white/5 rounded-3xl p-6 border border-white/5 flex-none">
                                ...
                                <div className="pt-4 border-t border-white/5"><SubjectDonut sessions={selectedDateSessions} subjects={subjects} /></div>
                             </div>
                ...
                {activeTab === 'settings' && (
                    <div className="h-full w-full overflow-y-auto custom-scrollbar animate-in fade-in zoom-in-95 duration-500">
                        <div className="w-full max-w-6xl mx-auto grid grid-cols-1 xl:grid-cols-2 gap-8 p-4 md:p-8">
                            {/* Reusing existing Settings content structure, just ensured styling is consistent */}
                             <div className="flex flex-col gap-8">
                                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 flex flex-col relative overflow-hidden group">
                                     ...
                                     <div className="relative z-10 flex flex-col h-full">
                                         <div className="flex items-center gap-4 mb-6 flex-none">
                                             <div className={`p-3 bg-${accent}-500/20 rounded-2xl text-${accent}-400`}><Palette size={28} /></div>
                                             <div><h3 className="text-2xl font-bold text-white">Personalization</h3><p className="text-slate-400 text-sm">Theme & Wallpaper</p></div>
                                         </div>
                                         <div className="mb-8">
                                             <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Accent Color</p>
                                             <div className="grid grid-cols-6 gap-3 w-full max-w-md">
                                                {ACCENT_COLORS.map(color => (
                                                    <button key={color.id} onClick={() => setAccent(color.id)} className={`relative aspect-square rounded-full transition-all duration-300 flex items-center justify-center group/btn ${color.colorClass} ${accent === color.id ? 'ring-4 ring-white/20 scale-110 shadow-xl' : 'opacity-60 hover:opacity-100 hover:scale-105'}`} title={color.label}>
                                         ...
                                         <div>
                                            <div className="flex items-center justify-between mb-3"><p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Wallpaper</p><button onClick={handleShowWallpaperToggle} className={`flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg transition-all ${showWallpaperOnHome ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/20' : 'bg-slate-800 text-slate-500 border border-slate-700 hover:text-slate-300'}`}><Layout size={14} /> Show on Dashboard</button></div>
                                            <div className="grid grid-cols-3 gap-3 mb-4">
                                                {WALLPAPERS.map(wp => (
                                                    <button key={wp.id} onClick={() => handleWallpaperChange(wp.url)} className={`relative aspect-video rounded-xl overflow-hidden border-2 transition-all group/wp ${wallpaper === wp.url ? `border-${accent}-500 ring-2 ring-${accent}-500/50` : 'border-transparent hover:border-white/20'}`}>
                                            ...
                                            <input type="text" value={wallpaper} onChange={(e) => handleWallpaperChange(e.target.value)} placeholder="Or paste custom image URL..." className="w-full bg-slate-900/50 border border-white/10 rounded-xl py-3 pl-4 pr-4 text-xs text-white focus:outline-none focus:border-white/20 transition-colors shadow-inner" />
                                         </div>
                                     </div>
                                </div>
                                <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 backdrop-blur-md rounded-[2.5rem] border border-indigo-500/30 p-8 relative overflow-hidden group flex-none">
                                    ...
                                    <div className="flex items-center justify-between relative z-10 mb-4">
                                        <div className="flex items-center gap-4"><div className="p-3 bg-indigo-500/20 rounded-2xl text-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.3)]"><Rocket size={24} /></div><div><h3 className="text-xl font-bold text-white">Space Mode</h3><p className="text-slate-400 text-sm flex items-center gap-1.5 mt-1"><Globe size={12} className="text-indigo-400" /> Live from the ISS</p></div></div>
                                    ...
                                    <div className="relative z-10 space-y-2"><input type="text" value={spaceVideoUrl} onChange={(e) => handleSpaceUrlChange(e.target.value)} placeholder="Paste YouTube Stream URL..." className="w-full bg-slate-950/50 border border-indigo-500/30 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-indigo-400 transition-colors placeholder:text-slate-600" /><div className="flex justify-between items-center text-xs"><span className="text-indigo-300/50">Supports standard YouTube video links</span><button onClick={() => handleSpaceUrlChange(DEFAULT_ISS_URL)} className="flex items-center gap-1.5 text-slate-400 hover:text-indigo-300 transition-colors font-medium"><RotateCcw size={12} /> Reset to NASA</button></div></div>
                                </div>
                                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 relative overflow-hidden group flex-none">
                                    <div className="relative z-10"><div className="flex items-center justify-between"><div className="flex items-center gap-4"><div className="p-3 bg-indigo-500/20 rounded-2xl text-indigo-400"><ImageIcon size={24} /></div><div><h3 className="text-xl font-bold text-white">Zen Mode</h3><p className="text-slate-400 text-sm">Use wallpaper for immersive timer</p></div></div><button onClick={handleZenToggle} className={`transition-colors ${enableZenMode ? `text-${accent}-400` : 'text-slate-600 hover:text-slate-400'}`}>{enableZenMode ? <ToggleRight size={32} /> : <ToggleLeft size={32} />}</button></div></div>
                                </div>
                            </div>
                            <div className="flex flex-col gap-8">
                                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 flex flex-col relative overflow-hidden group flex-none">
                                    <div className="relative z-10">
                                        <div className="flex items-center gap-4 mb-6"><div className="p-3 bg-blue-500/20 rounded-2xl text-blue-400"><Database size={24} /></div><div><h3 className="text-xl font-bold text-white">Data Management</h3><p className="text-slate-400 text-sm">Backup or restore your progress</p></div></div>
                                ...
                                <button onClick={() => setIsSubjectManagerOpen(true)} className="bg-white/5 hover:bg-white/10 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 text-left transition-all group relative overflow-hidden flex flex-col justify-center min-h-[160px] flex-none">
                                    ...
                                    <div className="relative z-10 flex items-center justify-between w-full">
                                        <div className="flex items-center gap-5"><div className={`p-4 bg-${accent}-500/20 rounded-2xl text-${accent}-400 group-hover:scale-110 group-hover:rotate-180 transition-all duration-500`}><Settings size={32} /></div><div><h3 className="text-2xl font-bold text-white mb-1 group-hover:translate-x-1 transition-transform">Subject Manager</h3><p className="text-slate-400 group-hover:text-slate-300 transition-colors">Configure subjects & colors</p></div></div>
                                ...
                                <div className="bg-red-500/5 backdrop-blur-xl border border-red-500/10 rounded-[2.5rem] p-8 flex flex-col relative overflow-hidden hover:border-red-500/20 transition-colors">
                                    ...
                                    <div className="mt-8 text-center flex-none"><p className="text-slate-700 text-[10px] font-mono font-medium tracking-widest uppercase hover:text-slate-500 transition-colors cursor-default select-none">OMNIFOCUS v2.0 • Built by Abu Dhabi</p></div>
                                </div>
                            </div>
                        </div>
                    </div>

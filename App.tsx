...
import { StudySession, Subject, DEFAULT_SUBJECTS, Task, Exam, isHexColor } from './types';
...
const App: React.FC = () => {
  ...
  const dailyTotalMs = useMemo(() => allSessions.filter(s => s.dateString === selectedDate).reduce((acc, curr) => acc + curr.durationMs, 0), [allSessions, selectedDate]);
  ...
  const SettingsContent = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto w-full">
        ...
        <div className="p-8 bg-gradient-to-br from-slate-800 to-slate-900/80 backdrop-blur-xl rounded-3xl border border-white/10 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl relative overflow-hidden group">
             <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
      ...
      <nav className="flex flex-col w-20 xl:w-64 flex-none py-6 h-full max-h-screen relative z-50">
         <div className="flex xl:justify-start justify-center px-4 mb-8 flex-none">
             <div className="flex items-center gap-3">
                <div className={`w-10 h-10 bg-${accent}-500/20 backdrop-blur-xl border border-white/10 rounded-xl flex items-center justify-center shadow-lg shadow-${accent}-500/20 flex-none`}>
                    <Zap size={20} className={`text-${accent}-400 fill-${accent}-400`} />
                </div>
                <h1 className="hidden xl:block font-bold text-lg tracking-tight text-white">OMNIFOCUS</h1>
             </div>
         </div>

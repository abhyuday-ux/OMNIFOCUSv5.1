...
class LocalDB {
  ...
  async updateUserRevenue(addedDurationMs: number) {
      if (!this.userId) return;
      const userRef = doc(db, 'user_profiles', this.userId);
      try {
      ...
      return onSnapshot(q, async (snapshot) => {
          ...
          for (const f of friendDocs) {
              if (f.status === 'accepted' || f.status === 'pending_received') {
                  ...
                  if (pSnap.exists()) f.profile = pSnap.data() as UserProfile;
              }
              friends.push(f);
          }
          callback(friends);
  ...
  async saveSession(session: StudySession): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
          ...
          this.updateUserRevenue(session.durationMs);
          resolve();
      };
      request.onerror = () => reject(request.error);
    });
    ...
    return new Promise((resolve, reject) => {
        ...
        getReq.onsuccess = () => {
            ...
            request.onsuccess = () => {
                ...
                if (session) this.updateUserRevenue(-session.durationMs);
                resolve();
            };
            request.onerror = () => reject(request.error);
        };

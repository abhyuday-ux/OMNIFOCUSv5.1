...
class LocalDB {
  ...
  startRealtimeSync() {
      ...
      const collections = [STORE_SESSIONS, STORE_SUBJECTS, STORE_GOALS, STORE_TASKS, STORE_EXAMS, STORE_JOURNAL, STORE_CHATS, STORE_CUSTOM_SOUNDS];
      ...
      const unsubSettings = onSnapshot(settingsRef, (docSnap) => {
          if (docSnap.exists()) {
              ...
              // Only clear if doc explicitly doesn't exist (reset), otherwise keep local
              // Actually, if doc is deleted, we should respect that.
  ...
  async saveCustomSound(item: CustomSound) { await this.saveToStore(STORE_CUSTOM_SOUNDS, item); }
  async deleteCustomSound(id: string) { await this.deleteFromStore(STORE_CUSTOM_SOUNDS, id); }

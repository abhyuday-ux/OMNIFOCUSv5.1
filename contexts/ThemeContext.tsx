...
export const ThemeProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleStorageUpdate = () => {
        const updated = localStorage.getItem('omni_theme_accent');
        if (updated) setAccent(updated as AccentColor);
    };

    window.addEventListener('omni_sync_complete', handleStorageUpdate);
    window.addEventListener('storage', handleStorageUpdate);

    return () => {
        window.removeEventListener('omni_sync_complete', handleStorageUpdate);
